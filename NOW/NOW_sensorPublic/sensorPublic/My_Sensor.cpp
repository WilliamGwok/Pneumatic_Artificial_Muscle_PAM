...
extern KF_Struct ay;
extern KF_Struct az;
extern KF_Struct grx;
extern KF_Struct gry;
extern KF_Struct grz;
static float i = 0;
static float roll = 0;
...
void initSensor()
  ...
  *  日    期： 2023.8.29
 **************************************************************/
 void getData(String name, int port) {
  ...
  *  日    期： 2023.8.29
 **************************************************************/
String GetInString(String sensorName, int temp, int system) 
{
...
String GetJSONString(String gettime, String sensorName, float x, float y , float z, float w, float accx, float accy, float accz, float gyrox, float gyroy, float gyroz, int system, float tag, float power, float SendRate) 
{
    StaticJsonDocument<1024> staticJsonDocument;//创建静态的JSON文档对象staticJsonDocument，缓冲区大小为1024
    staticJsonDocument["t"] = gettime;       //设置本地时间
    ...
    staticJsonDocument["tag"] = String(tag, 2);       // 发生速率校验
    staticJsonDocument["p"] = String(power, 2);        //电压
    staticJsonDocument["Hz"] = String(SendRate, 2);       // 发送频率
...
void UDP_Send_Data(String name, int port, uint8_t system, float vol, float w, float x, float y, float z)
{
  // 将上述数据打包为json格式
  i++;//发生速率标志位
  String gettime = Get_localtime();
  //Serial.print("CPU clock frequency: ");
  //Serial.print(F_CPU);
  //Serial.println(" Hz");
  
  json = GetJSONString (gettime, name, x, y, z, w, accx, accy, accz, gyrox, gyroy, gyroz, system, i, vol, MySendRate);

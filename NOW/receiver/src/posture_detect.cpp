...
float w[dataSize], x[dataSize], y[dataSize], z[dataSize];
float q_current[dataSize][4];
float q_ref[4] = {0, 0, 0, 0};  // 参考四元数
...
/**
  ...
  * @retval None
  */
void judgeArmPosture(float q_ref[], float q_current[], float &dot_product, float &theta) {
    // 计算四元数点积
    dot_product = q_ref[0] * q_current[0] + q_ref[1] * q_current[1] + q_ref[2] * q_current[2] + q_ref[3] * q_current[3];

    // 计算旋转角度（度）
    theta = abs(2 * acos(dot_product) * 180.0 / PI);
}
...
{
    float dot_product, theta;
    judgeArmPosture(q_ref, q_current, dot_product, theta);
